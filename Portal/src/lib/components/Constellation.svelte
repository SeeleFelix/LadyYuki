	...
	function animate() {
		...
		for (const line of state.lines) {
			...
			for (let d = 0; d < 2; d++) {
				...
				ctx.beginPath(); ctx.arc(dx, dy, 6, 0, Math.PI * 2); ctx.fillStyle = eg; ctx.fill();
		...
		for (const star of state.stars) {
			...
			ctx.beginPath(); ctx.arc(star.x, star.y, oR, 0, Math.PI * 2); ctx.fillStyle = og; ctx.fill();
			...
			ctx.beginPath(); ctx.arc(star.x, star.y, mR, 0, Math.PI * 2); ctx.fillStyle = mg; ctx.fill();

			// Selection ring
			if (isSelected) {
				ctx.beginPath(); ctx.arc(star.x, star.y, mR + 2, 0, Math.PI * 2);
				ctx.strokeStyle = `rgba(255,220,150,${0.4 + twinkle * 0.3})`;
				ctx.lineWidth = 1.5; ctx.stroke();
			}

			// Core
			ctx.beginPath(); ctx.arc(star.x, star.y, cs, 0, Math.PI * 2);
			ctx.fillStyle = `rgba(255,255,255,${bright})`; ctx.fill();

			// Inner
			ctx.beginPath(); ctx.arc(star.x, star.y, cs * 0.4, 0, Math.PI * 2);
...
<canvas bind:this={canvas} class="fixed inset-0 w-full h-full constellation-canvas" />

<style>
	.constellation-canvas { z-index: 1; cursor: pointer; }
</style>
