<script lang="ts">
  ...
  function drawNebulaBlobs(time: number) {
    for (const blob of nebulaBlobs) {
      ...
      const noiseX =
        (noise(time * blob.speed * 3, blob.noiseOffset, 0) - 0.5) * 80;
      const noiseY =
        (noise(time * blob.speed * 3, blob.noiseOffset, 100) - 0.5) * 80;
      ...
      gradient.addColorStop(
        0,
        `rgba(${blob.color.r},${blob.color.g},${blob.color.b},${blob.opacity * pulse})`,
      );
      gradient.addColorStop(
        0.35,
        `rgba(${blob.color.r},${blob.color.g},${blob.color.b},${blob.opacity * pulse * 0.5})`,
      );
      gradient.addColorStop(
        0.7,
        `rgba(${blob.color.r},${blob.color.g},${blob.color.b},${blob.opacity * pulse * 0.15})`,
      );
